...
public:
	vector <string> build(vector <int> values) {
		vector <string> ans;
		int occ[10];
		for (int i = 0; i < values.size(); ++i) occ[values[i]]++;
		int h = 0;
		for (int i = 0; i < 10; ++i) h = max(h,occ[i]);
		for (int i = 0; i <= h; ++i) ans.push_back("..........");
		for (int i = 0; i < 10; ++i) for (int j = 0; j < occ[i]; ++j) ans[j][i] = 'X';
