...
int main(){
	...
	while (tc--){
		...
		A = point(x_1,y_1); B = point(x_1,y_2); C = point(x_2,y_1); D = point(x_2,y_2);
		a = segment(A,B); b = segment(B,D); c = segment(D,C); d = segment(C,A);
		s = segment(point(x_3,y_3),point(x_4,y_4));
		if (intersect(a,s)||intersect(b,s)||intersect(c,s)||intersect(d,s)) printf("YA\n");
			else printf("TIDAK\n");
	}
	return 0;
}
