...
int calc(int dd, int MM, int yy) {
	set<int> s;
	REP(ty, 2) REP(iM, 2) REP(id, 2) REP(iy, 2) {
		string date = "";
		if (ty) {
			// Month
			sprintf(buffer, iM ? "%02d" : "%d", MM);
		...
		if (SZ(date) == 4) {
			for (int i = 0; i < nhours0; ++i) {
				s.insert(hours0[i] * 3600 + mmss);
			}
		} else if (SZ(date) == 5) {
			int x = date[4] - '0';
			for (int i = 0; i < 5; ++i) {
                if (hours1[x][i] < 0) break;
				s.insert(hours1[x][i] * 3600 + mmss);
			}
		} else {
			set<int> huba;
			if (isPalindrome(date.substr(5, SZ(date) - 5))) {
				int x = date[4] - '0';
				if (x == 0) {
					huba.insert(0);
				} else {
			...
			if (isPalindrome(date.substr(6, SZ(date) - 6))) {
				int x = date[4] - '0';
				int y = date[5] - '0';
				int hh = 10 * y + x;
				if (0 <= hh && hh <= 23) {
					huba.insert(hh);
					if (hh != 0 && 0 <= hh + 12 && hh + 12 <= 23) {
						huba.insert(hh + 12);
					}
					if (hh == 12) {
						huba.insert(0);
