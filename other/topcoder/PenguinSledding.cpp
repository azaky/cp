...
public:
	long long countDesigns(int numCheckpoints, vector <int> checkpoint1, vector <int> checkpoint2) {
		vector<int> a = checkpoint1, b = checkpoint2;
		int n = numCheckpoints, m = (int)checkpoint1.size();
		long long ans = 0;
		//none
		ans += 1LL;
		//alone
		ans += (long long)m;
		//star > 1
		int deg[55]; memset(deg,0,sizeof(deg));
		for(int i=0; i < m; ++i) ++deg[a[i]], ++deg[b[i]];
		for(int i=1; i <= n; ++i){
			long long temp = (long long)deg[i];
			ans += (1LL << temp) - 1 - temp;
		}
		//triangle
		int con[55][55]; memset(con,0,sizeof(con));
		for(int i=0; i < m; ++i) con[a[i]][b[i]] = con[b[i]][a[i]] = 1;
		long long tri = 0;
		for (int i=1; i <= n; ++i) for (int j=1; j <= n; ++j) for (int k=1; k <= n; ++k)
			if (con[i][j] && con[j][k] && con[k][i]) tri++;
		ans += tri/6LL;
		return ans;
