...
int main() {
  ...
  sort(projects.begin(), projects.end(), [](Project& a, Project& b) {
    ...
    vector<int> aSkills(a.skills.size());
    vector<int> bSkills(b.skills.size());
    for (int i = 0; i < a.skills.size(); ++i) {
      aSkills[i] = a.skills[i].second;
    }
    for (int i = 0; i < b.skills.size(); ++i) {
      bSkills[i] = b.skills[i].second;
    }
    sort(aSkills.begin(), aSkills.end());
    sort(bSkills.begin(), bSkills.end());
  ...
  for (int i = 0; i < P; ++i) {
    ...
    int bestNextProject = -1;

    // Pick projects that "has no dependency". Some kind of toposort?
    for (int u = 0; u < P; ++u) {
      if (done[projects[u].name]) {
        continue;
      }
      ...
      for (int v = 0; v < P; ++v) {
        if (done[projects[v].name] || u == v) {
          continue;
        }
        bool isDependency = false;
        for (auto& su : projects[u].skills) {
          for (auto& sv : projects[v].skills) {
            if (su.first == sv.first) {
    ...
    if (bestNextProject != -1) {
      debug("project[%d]: %s\n", bestNextProject, projects[bestNextProject].name.c_str());
      for (auto &s : projects[bestNextProject].skills) {
        debug("  %s: %d\n", s.first.c_str(), s.second);
      }
    }

    // If all cycles, then pick the next one with heuristics.

    if (bestNextProject == -1) {
      // debug("all cycles, picking by heuristics\n");
      ...
      for (int u = 0; u < P; ++u) {
        ...
        for (int k = 0; k < project.skills.size(); ++k) {
          ...
          for (int j = 0; j < C; ++j) {
            ...
            if (best == -1 ||
              ...
              (clevel == contributors[best].skills[skill] && lastAvailable[j] < lastAvailable[best])) {
              best = j;
            }
          }
          if (best == -1) {
          ...
          assigned[best] = true;
          if (contributors[best].skills[skill] == level) {
            cur.first ++;
            // cur.first += level;
          } else {
            // cur.push_back(1000);
            cur.second += level - contributors[best].skills[skill];
          }
        }
        ...
        // if (bestNextProject == -1 || cur < bestScore) {
        //   bestNextProject = u;
        //   bestScore = cur;
        // }

        // if (i == 0 || i == 1 || i == 6) {
        //   debug("project[%d]: (%d, %d)\n", u, cur.first, cur.second);
        // }
        auto better = [](pair<int, int> a, pair<int, int> b) {
          // always favor nonzero first
        ...
        if (bestNextProject == -1 || better(cur, bestScore)) {
          bestNextProject = u;
          bestScore = cur;
        }
      }
    ...
    if (bestNextProject == -1) {
      debug("After %d projects, no more projects to assign.\n", i);
      // debug("project[%d]: %s not assigned\n", i, projects[i].name.c_str());
      break;
    }

    Project& project = projects[bestNextProject];
    // debug("Next project: %s, upskill: (%d, %d)\n", projects[bestNextProject].name.c_str(), bestScore.first, bestScore.second);
