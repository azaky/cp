...
int main() {
  ...
  for (int i = 0; i < P; ++i) {
    ...
    for (int u = 0; u < P; ++u) {
      if (done.find(projects[u].name) != done.end()) {
        continue;
      }
      ...
      for (int k = 0; k < project.skills.size(); ++k) {
        ...
        assigned[best] = true;
        skillGap += contributors[best].skills[skill] - level;
      }
      ...
      if (bestNextProject == -1 || skillGap < minSkillGap) {
        bestNextProject = u;
        minSkillGap = skillGap;
      }
    }
    ...
    debug("Next project: %s, skill gap: %d\n", projects[bestNextProject].name.c_str(), minSkillGap);
