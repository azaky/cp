...
int main() {
  ...
  map<string, bool> projectPossible;
  for (int i = 0; i < P; ++i) {
    Project& project = projects[i];
    bool possible = true;
    map<string, int> requiredSkillCount;
    for (auto& s : project.skills) {
      requiredSkillCount[s.first]++;
    }
    for (auto& s : requiredSkillCount) {
      if (skillCount[s.first] < s.second) {
        possible = false;
        break;
      }
    }
    projectPossible[project.name] = possible;
  }
  vector<Project> possibleProjects;
  for (int i = 0; i < P; ++i) {
    if (projectPossible[projects[i].name]) {
      possibleProjects.push_back(projects[i]);
    }
  }
  projects = possibleProjects;
  P = projects.size();
  debug("%lu projects are possible\n", projects.size());
  ...
  sort(projects.begin(), projects.end(), [](Project& a, Project& b) {
    int gainA = a.B * a.S / a.D;
    int gainB = b.B * b.S / b.D;
    return gainA > gainB;

    int maxA = 0;
    int maxB = 0;
    for (auto& s : a.skills) {
      maxA = max(maxA, s.second);
    }
    for (auto& s : b.skills) {
      maxB = max(maxB, s.second);
    }
    if (maxA != maxB) {
      return maxA > maxB;
    }
