...
vector<int> centerCars;

int simulate(vector<vector<pair<int, int>>> ans) {
  volumes.resize(S);
  fill(volumes.begin(), volumes.end(), 0);

  centerCars.resize(S);
  fill(centerCars.begin(), centerCars.end(), -1);
  ...
  for (int t = 0; t <= D; ++t) {
    ...
    while (!qevents[t].empty()) {
      ...
      if (streets[streetId].E == 499) {
        if (centerCars[streetId] == -1) {
          centerCars[streetId] = t;
        }
      }
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < V; ++i) {
    ...
    for (int j = 0; j+1 < cars[i].path.size(); ++j) {
      if (streets[cars[i].path[j]].E == 499) {
        passCenter = true;
      }
    }
    if (passCenter) {
      carsCenter.push_back(i);
    }
  }

  vector<int> centerScores(S);
  for (int i = 0; i < V; ++i) {
    for (int j = 0; j + 1 < cars[i].path.size(); ++j) {
      if (streets[cars[i].path[j]].E == 499) {
        centerScores[cars[i].path[j]] += cars[i].maxScore;
      }
    }
  }
  ...
  int center = 0;
  for (int s : adj[499]) {
    if (used[s]) debug(" %d", used[s]);
    center += used[s];
  }
  debug("\n");
  debug("center total: %d\n", center);
  debug("center unique: %d\n", (int)carsCenter.size());
  debug("center scores:");
  // sort(centerScores.begin(), centerScores.end());
  for (int s : centerScores) {
    if (s) debug(" %d", s);
  }
  debug("\n");
  ...
  for (int iter = 1; iter <= 1; ++iter) {
    ...
    for (int inneriter = 1; inneriter <= 5; ++inneriter) {
      ...
      for (int v = 0; v < I; ++v) {
        ...
        for (auto s : adj[v]) {
          ...
          if (vcount > 2 && centerScores[s] < 900) {
            schedule.pop_back();
          }
        }
        ...
        // if (vcount > 2) {
        //   stable_sort(schedule.begin(), schedule.end(), [&](const pii &a, const pii &b) -> bool {
        ...
        if (vcount > 2) {
          ...
          for (pii x : schedule) {
            debug(" %d", x.second);
            totalschedule += x.second;
          }
          debug("\n");
          debug("\ttotal schedule = %d\n", totalschedule);
          debug("\tlength schedule = %d\n", (int)schedule.size());
        }
      ...
      for (int i = 0; i < S; ++i) {
        ...
        if (s != -1) debug(" (%d, %d)", centerCars[i], s);
      }
      debug("\n\n");

      // rearrange ans[499]
      int deg = ans[499].size();
      vector<pii> sched;
      vector<bool> done(deg, false);

      int lastt = 0;
      while (sched.size() < deg) {
        // find best candidate
        // first, find the one that has closest time.
        // second, idk, just compare by centerScores and size I guess.
        int best = -1;
        for (int i = 0; i < deg; ++i) {
          if (done[i]) continue;
          if (best == -1) {
          ...
          int s = ans[499][i].first;
          int t = ans[499][i].second;
          if (centerCars[s] > lastt) continue;
          int difft = lastt - centerCars[s];
          int diffbest = lastt - centerCars[ans[499][best].first];
          if (diffbest < 0 || difft < diffbest) {
            best = i;
            continue;
          }
          if (difft > diffbest) continue;
          if (t < ans[499][best].second) {
            best = i;
            continue;
          }
          // just pick randomly
          ...
          // if (centerScores[s] > centerScores[ans[499][best].first]) {
          //   best = i;
          //   continue;
          // }
        }
        assert(best != -1);
        int s = ans[499][best].first;
        int t = ans[499][best].second;
        lastt += t;
        done[best] = true;
