...
int main(){
	...
	r = 505;
	REP(i,n){
		db x = (db)(rand()%10000 + 550);
		db y = (db)(rand()%10000 + 550);
		if (rand()%2) x = -x;
		if (rand()%2) y = -y;
	...
	/* print to stdout */
	printf("%d %.0lf\n",n,r);
	REP(i,n) printf("%.0lf %.0lf\n",p[i].x,p[i].y);
	/* calc answer */
	ans = 0;
	REP(i,n-1) FOR(j,i+1,n-1){
		if (intersectionSegmentCircle(segment(p[i],p[j]),circle(point(0,0),r)).empty()) ans++;
		if (isTangent(segment(p[i],p[j]),circle(point(0,0),r))) fprintf(stderr,"tangent lines found!\n");
	}
