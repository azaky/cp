...
int main(){
	...
	polygon hulla = convexHull(a);
	polygon hullb = convexHull(b);
	
	if (hulla.front() == hulla.back()) hulla.pop_back();
	if (hullb.front() == hullb.back()) hullb.pop_back();
	...
	map < int,vector<db> > pa, pb; // x maps y
	
	REP(i,n) pa[(int)a[i].x].PB(a[i].y);
	REP(i,n) pb[(int)b[i].x].PB(b[i].y);
	
	//FORIT(it,hulla) printf("(%.2lf,%.2lf) ",it->x,it->y); printf("\n");
	//FORIT(it,hullb) printf("(%.2lf,%.2lf) ",it->x,it->y); printf("\n");
	
	int ansa = 0;
	int mina = 40000, maxa = -40000;
	REP(i,n){
		MN(mina,(int)a[i].x);
		MX(maxa,(int)a[i].x);
	}
	int iup = na-1, idown = 0;
	
	FOR(x,mina,maxa) {
		//adjust iup
		while (x > (int) hulla[iup].x || x < (int) hulla[(iup+1) % na].x) iup--;
		//adjust idown
		while (x < (int) hulla[idown].x || x > (int)hulla[(idown+1) % na].x) idown++;
		//l : x = x
		segment l = segment (point((db)x,0.),point((db)x,1.));
		segment sup = segment(hulla[iup],hulla[(iup+1) % na]);
		segment sdown = segment(hulla[idown],hulla[(idown+1) % na]);
		db yup = isParallel(l, sup) ? 40000. : intersection(l, sup).y;
		db ydown = isParallel(l, sdown) ? -40000. : intersection(l, sdown).y;
	...
	FOR(x,minb,maxb) {
		//adjust iup
		while (x > (int) hullb[iup].x || x < (int) hullb[(iup+1) % nb].x) iup--;
		//adjust idown
		while (x < (int) hullb[idown].x || x > (int)hullb[(idown+1) % nb].x) idown++;
		//l : x = x
		segment l = segment (point((db)x,0.),point((db)x,1.));
		segment sup = segment(hullb[iup],hullb[(iup+1) % nb]);
		segment sdown = segment(hullb[idown],hullb[(idown+1) % nb]);
		db yup = isParallel(l, sup) ? 40000. : intersection(l, sup).y;
		db ydown = isParallel(l, sdown) ? -40000. : intersection(l, sdown).y;
