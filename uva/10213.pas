...
begin
  ...
  for tc := 1 to s do
    begin
      readln(n);
      a[1]:=n; a[2]:=max(0,n-1); a[3]:=max(0,n-2); a[4]:=max(0,n-3);
      ans := num(1);
      for i:=1 to 4 do
        if a[i] mod 2=0 then
          begin a[i]:=a[i] div 2; break; end;
      ans := ans+(num(a[1])*num(a[2]));
      for i:=1 to 4 do
        if a[i] mod 3=0 then
          begin a[i]:=a[i] div 3; break; end;
      for i:=1 to 4 do
        if a[i] mod 2=0 then
          begin a[i]:=a[i] div 2; break; end;
      for i:=1 to 4 do
        if a[i] mod 2=0 then
          begin a[i]:=a[i] div 2; break; end;
      ans := ans+(num(a[1])*num(a[2])*num(a[3])*num(a[4]));
      println(ans);
    end;
end.
