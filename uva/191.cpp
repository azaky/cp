...
int main(){
	...
	while (ntc--){
		...
		point a (xleft,ytop);
		point b (xright,ytop);
		point c (xright,ybottom);
		point d (xleft,ybottom);

		polygon rect;
		rect.PB(a); rect.PB(b); rect.PB(c); rect.PB(d);

		if (isPointInsidePolygon(p,rect) || isPointInsidePolygon(q,rect) || isIntersecting(x,segment(a,b)) || isIntersecting(x,segment(b,c)) || isIntersecting(x,segment(c,d)) || isIntersecting(x,segment(a,d))) puts("T"); else puts("F");
	}
