...
polygon cut(polygon p, segment l, point f){
	int k = 0, n = p.size();
	polygon ret;
	while (!isOnSameSide(p[k],f,l)) k++;
	for (int start = k, j = (k+1)%n; ; k = j++, j %= n){
		/* assume that p[k] inside _p */
		ret.PB(p[k]);
		/* now examining p[j] and segment between them */
		if (isOnSameSide(p[j],f,l)){
			if (j == start) break;
			continue;
		}
		segment side(p[k],p[j]);
		ret.PB(intersection(segment(p[k],p[j]),l));
		while (!isOnSameSide(p[j],f,l)) j++, j %= n;
		k = (j+n-1)%n;
		ret.PB(intersection(segment(p[k],p[j]),l));
		if (j == start) break;
	}
...
int main(){
	...
	double w, h;
	point fountain;
	for (int itc = 1; scanf("%d%lf%lf%lf%lf",&n,&w,&h,&fountain.x,&fountain.y) == 5; ++itc){
		polygon garden;
		garden.PB(point(0,0));
		garden.PB(point(w,0));
		garden.PB(point(w,h));
		...
		REP(_i,n){
			...
			garden = cut(garden,l,fountain);
		}
		printf("Case #%d: %.3lf\n",itc,polygonArea(garden));
	}
