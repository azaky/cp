...
int main(){
	...
	while (cin >> m.x >> m.y){
		...
		FOR(i,0,n){
			cin >> p.x >> p.y;
			if (i){
				point x = projection(m,segment(p,q));
//				printf("Projection of (%.2lf,%.2lf) onto line (%.2lf,%.2lf)-(%.2lf,%.2lf) is (%.2lf,%.2lf)\n",m.x,m.y,p.x,p.y,q.x,q.y,x.x,x.y);
				db _d = distToSegment(m,segment(p,q));
//				printf("distance to segment = %.3lf\n",_d);
				if (_d < d){
					d = _d;
					if (abs(dist(x,m) - d) < EPS) ans = x;
					else if (abs(dist(p,m) - d) < EPS) ans = p;
					else if (abs(dist(q,m) - d) < EPS) ans = q;
				}
			}
			q = p;
		}
