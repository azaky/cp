...
int main(){
	...
	FOR(itc, 1, ntc) {
		...
		int l = 0, r = 20000000;
		int source = 0, sink = n + 2 * m + 1, nnodes = sink + 1;
		while (l < r) {
			int mid = (l + r) / 2;
			...
			FOR(i, 1, n) res[source][i] = 1;
			FOR(i, n + 1, sink - 1) res[i][sink] = 1;
			if (maxFlowFordFulkerson(nnodes, source, sink) >= k) {
				r = mid;
			} else {
				l = mid + 1;
			}
