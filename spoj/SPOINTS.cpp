...
int main(){
	...
	while (cin >> n >> m && n && m){
		...
		if (isPointInsidePolygon(white[0],black) || isPointInsidePolygon(black[0],white)){
			puts("NO");
		}
		else {
			bool motong = false;
			REP(i,n) if (!motong) REP(j,m){
				if (isIntersecting(segment(white[i],white[(i+1)%n]),segment(black[j],black[(j+1)%m]))){
					motong = true;
					break;
				}
			}
			if (motong) puts("NO"); else puts("YES");
		}
	}
