...
//				presults[it->second.id].user[ut->userid].bonus= -0.5 >? (0.11-0.01*cr);
	...
	fprintf(output, "%d\n", 1);	// number of rankings
	{
		...
		while(!res.empty())
		{
			...
			fprintf(output, "%d\n<a href='/%s/users/%s'>%s</a>\n", rnum, BASE_PATH,users[it.userid].login.c_str(), users[it.userid].name.c_str());
			...
			for(pl::const_iterator pt = psets[i].problems.begin(); pt != psets[i].problems.end(); pt++)
			{
				...
				{
					fprintf(output, "<font title='%d submissions'>%.2f</font>",
						uresults[it.userid].prob[*pt].tries,
//						presults[*pt].coef/presults[*pt].user[it.userid].rank);
						presults[*pt].user[it.userid].points);
