...
//				presults[it->second.id].user[ut->userid].bonus= -0.5 >? (0.11-0.01*cr);
	...
	fprintf(output, "%d\n", 1);	// number of rankings
	{
		...
		for(pl::const_iterator it = psets[i].problems.begin(); it != psets[i].problems.end(); it++)
			fprintf(output, "<a href='/%s/problems/%s'>%s</a>\n", BASE_PATH, problems[*it].code.c_str(),problems[*it].name.c_str());
		...
		while(!res.empty())
		{
			...
			for(pl::const_iterator pt = psets[i].problems.begin(); pt != psets[i].problems.end(); pt++)
			{
				...
				{
				  if (presults[*pt].user[it.userid].points==presults[*pt].coef)
				  {
				    fprintf(output, "<font color='#00CC00'><b>%.2f</b></font>",	presults[*pt].user[it.userid].points);
				    sss++;
                  }
                  else fprintf(output,"<font color='#FF0033'><b>%.2f</b></font>", presults[*pt].user[it.userid].points);
