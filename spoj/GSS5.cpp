...
int n, a[maxn], q, xl, xr, yl, yr;
node tree[4*maxn],kiri,kanan,tengah;
...
int main(){
	...
	while (ntc--){
		...
		q = getint();
		for (int i=1; i<=q; ++i){
			xl = getint(); yl = getint(); xr = getint(); yr = getint();
			//disjoint case
			if (yl < xr){
				kiri = query(1,xl,yl);
				kanan = query(1,xr,yr);
				tengah = query(1,yl+1,xr-1);
				printf("%d\n",kiri.su+tengah.sum+kanan.pre);
			}
			else{
				kiri = query(1,xl,xr-1);
				kanan = query(1,yl+1,yr);
				tengah = query(1,xr,yl);
				printf("%d\n",max4(kiri.su+tengah.pre,tengah.su+kanan.pre,kiri.su+tengah.sum+kanan.pre,tengah.mid));
			}
		}
	}
	return 0;
