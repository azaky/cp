...
vector<db> naiveMultiplication(vector<db> a, vector<db> b) {
	int na = SZ(a), nb = SZ(b);
	vector<db> result(na + nb - 1);
	for (int i = 0; i < na; ++i) {
		for (int j = 0; j < nb; ++j) {
			result[i + j] += a[i] * b[j];
		}
...
long long powerMod(long long a, long long b, long long mod) {
	if (b == 0) {
		...
		return mulmod(a, powerMod(a, b - 1, mod), mod);
	} else {
		return powerMod(mulmod(a, a, mod), b / 2LL, mod);
	}
}

bool rabinMillerPrimalityTest(long long n) {
	if ((n & 1) == 0) return n == 2;
	if (n == 1) return false;
	...
	for (int i = 0; i < 12; ++i) {
		if (a[i] >= n) break;
		// printf("testing for %lld\n", a[i]);
		long long ad = powerMod(a[i], d, n);
		if (ad != 1) {
			bool composite = true;
			for (int j = 0; j < s; ++j) {
				if (ad == n - 1) {
...
vector<db> getRandomDoubleVector() {
	vector<db> res;
	int n = 1000 + (rand() % 1000);
	REP(i, n) {
		int sign = rand() % 2 ? -1 : 1;
		db num = 1. * (rand() % 1000) / 1000 * sign;
		res.PB(num);
	}
	return res;
}
...
int main(){
	...
	for (int i = 1; i <= MAXP; ++i) {
		if (prime[i] ^ rabinMillerPrimalityTest(i)) {
	...
	// int n; db x;
	// vector<db> a, b;
	// // a = getRandomDoubleVector();
	// // b = getRandomDoubleVector();
	// scanf("%d",&n);
	// REP(i,n){
	// 	scanf("%lf",&x);
	// 	a.PB(x);
	...
	// vector<db> res2 = naiveMultiplication(a, b);
	// REP(i,SZ(res2)) printf("%g ",res2[i]); puts("");
	// if (SZ(res) != SZ(res2)) {
	// 	puts("Size mismatch");
	// 	return -1;
	// }
	// REP(i, SZ(res)) {
	// 	if (fabs(res[i] - res2[i]) > 1e-6) {
	// 		printf("%d Incorrect! %g vs %g\n", i, res[i], res2[i]);
	// 		return -1;
	// 	}
	// }
